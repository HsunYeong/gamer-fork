...
def yt_inline():
   ...
   if ('gamer', 'CR_E') in ds.derived_field_list:
      ...
      ds.add_field( ('gas', 'cosmic_ray_energy_density'), function=_cr_e, sampling_type='cell', units='erg/cm**3' )

      field = ('gas', 'cosmic_ray_energy_density')
      pz_cray = yt.ProjectionPlot( ds, 'z', field, center='c',
                                   data_source=box, width=(2*half_width, 'code_length' ), buff_size=(1024, 1024) )
      pz_cray.set_axes_unit( 'kpc' )
      pz_cray.set_unit( field, 'Msun/pc**2*km**2/s**2' )
      pz_cray.set_zlim( field, 1.0e+1, 1.0e+5 )
      pz_cray.set_cmap( field, 'plasma' )
      pz_cray.annotate_timestamp( time_unit='Myr', corner='upper_right' )
      pz_cray.save()

      pz_cray.annotate_particles( (2*half_width, 'code_length'), ptype='young_SNII', p_size=0.7, col='w', marker='o' )
      pz_cray.save( '%s_wYSNeII'%ds )
