...
def create_PDF( ds, obj, prefix, field_type, field_x, weighting_type, lim_x_min, lim_x_max, nbin, units_x ):
   ...
   assert weighting_type == 'volume' or weighting_type == 'mass' or weighting_type == 'dust_mass', 'Incorrect weighting type'
   ...
   pdf_y = arr_y / (sum(arr_y) * log_bin_w) if sum(arr_y) > 0 else np.zeros_like(arr_y)
...
def create_PhaseDiagram( ds, obj, prefix, field_type, weighting_type, nbin, x_lim_min, x_lim_max, y_lim_min, y_lim_max ):
   ...
   units_x = 'g/cm**3'
   units_y = 'K'
   units_w = 'Msun' if weighting_type == 'mass' or weighting_type == 'dust_mass' else 'kpc**3'
...
def plot_DGR( fig, pos_in_fig, indices, prefix, field_x, lim_x_min, lim_x_max, nbin, units_x ):
   ...
   for idx in indices[::-1]:
      ...
      if idx == indices[-1]:
         arr_xg = np.fromfile('./bins/%s%s_%s_weighted_%s_profile_x.bin'%(filename, prefix, 'mass',      field_x.capitalize()))
         arr_xd = np.fromfile('./bins/%s%s_%s_weighted_%s_profile_x.bin'%(filename, prefix, 'dust_mass', field_x.capitalize()))
         arr_yg = np.fromfile('./bins/%s%s_%s_weighted_%s_profile_y.bin'%(filename, prefix, 'mass',      field_x.capitalize()))
         arr_yd = np.fromfile('./bins/%s%s_%s_weighted_%s_profile_y.bin'%(filename, prefix, 'dust_mass', field_x.capitalize()))

         assert np.array_equal(arr_xg, arr_xd), 'arr_xg != arr_xd'
      else:
         try:
            arr_yg += np.fromfile('./bins/%s%s_%s_weighted_%s_profile_y.bin'%(filename, prefix, 'mass',      field_x.capitalize()))
            arr_yd += np.fromfile('./bins/%s%s_%s_weighted_%s_profile_y.bin'%(filename, prefix, 'dust_mass', field_x.capitalize()))
   ...
   arr_y = np.divide( arr_yd, arr_yg, out=np.zeros_like(arr_yg), where=arr_yg>0 )
   ...
   if pos_in_fig == 'x':
      ax.loglog( arr_xg, arr_y )

#      ax.set_title( 'PDF weighted by %s'%weighting_type )
      ax.set_ylabel( r'$Z_d$', fontsize='x-large')

      ax.set_xlim( lim_x_min, lim_x_max )
      ax.set_ylim( 1e-4, 2e-3 )

   if pos_in_fig == 'y':
      ax.loglog( arr_y, arr_xg )

      ax.yaxis.tick_left()
      ax.yaxis.set_label_position("left")
      ax.set_xlabel( r'$Z_d$', rotation=0, fontsize='x-large')
...
#  plot
   ...
   w_total_obj = zf.sum()
   zf = zf / w_total_obj if w_total_obj > 0 else np.zeros_like(zf)
...
def plot_PhaseDiagram_DGR( indices, prefix, nbin, x_lim_min, x_lim_max, y_lim_min, y_lim_max, title_time, fileout ):
...
#  plot
   ...
   for idx in indices[::-1]:
      ...
      if idx == indices[-1]:
         xf  = np.fromfile('./bins/%s%s_Temp-Dens_%s_weighted_PhaseDiagram_x.bin'%(filename, prefix, 'mass'))
         yf  = np.fromfile('./bins/%s%s_Temp-Dens_%s_weighted_PhaseDiagram_y.bin'%(filename, prefix, 'mass'))
         zf  = np.fromfile('./bins/%s%s_Temp-Dens_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, 'mass'     )).reshape(len(xf),len(yf))
         zd  = np.fromfile('./bins/%s%s_Temp-Dens_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, 'dust_mass')).reshape(len(xf),len(yf))
      else:
         try:
            zf += np.fromfile('./bins/%s%s_Temp-Dens_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, 'mass'     )).reshape(len(xf),len(yf))
            zd += np.fromfile('./bins/%s%s_Temp-Dens_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, 'dust_mass')).reshape(len(xf),len(yf))
   ...
   DGR = np.divide( zd,  zf, out = np.zeros_like(zf), where=zf>0 )

   FONT_SIZE = 18

   im = grid[0].axes.pcolormesh(xf, yf, DGR.T, shading='nearest', vmin=1.0e-4, vmax=2.0e-3, cmap=colormap, norm='log' )
   grid[0].axes.set_xlim( x_lim_min, x_lim_max )
   grid[0].axes.set_ylim( y_lim_min, y_lim_max )
   grid[0].axes.set_xscale('log')
   grid[0].axes.set_yscale('log')
   ...
   grid[0].axes.tick_params( which='both', direction='in', labelsize=FONT_SIZE )
   cbar = fig.colorbar(im, ax=grid[0].axes, cax=grid.cbar_axes[0], pad=0.0)
   cbar.set_label('Dust to Gas Ratio', fontsize=FONT_SIZE)
   cbar.ax.tick_params( which='both', direction='in', labelsize=FONT_SIZE )
   ...
   plot_DGR( fig, 'x', indices, prefix, 'density', x_lim_min, x_lim_max, nbin, units_x )
   plot_DGR( fig, 'y', indices, prefix, 'T'      , y_lim_min, y_lim_max, nbin, units_y )
   fig.savefig( './imgs_pd/fig_%s%s_Temp-Dens_%s_weighted_PhaseDiagram.png'%(fileout, prefix, 'DGR') )
   plt.close()
