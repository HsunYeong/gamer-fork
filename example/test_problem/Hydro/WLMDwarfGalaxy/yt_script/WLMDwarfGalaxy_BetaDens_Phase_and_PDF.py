...
def create_PDF( ds, obj, prefix, field_type, field_x, weighting_type, lim_x_min, lim_x_max, nbin, units_x ):
   ...
   field_out = 'plasma_beta_dual' if field_x == 'plasma_beta' else field_x

   prof = yt.create_profile( obj, (field_type, field_out), fields=(field_type, weighting_type),
                             weight_field=None,
                             n_bins=nbin,
                             logs={(field_type, field_out): True, (field_type, weighting_type): True},
                             units={(field_type, field_out): units_x},
                             extrema={(field_type, field_out): (lim_x_min, lim_x_max)},
                             fractional=False,
                           )
...
def create_PhaseDiagram( ds, obj, prefix, field_type, weighting_type, nbin, x_lim_min, x_lim_max, y_lim_min, y_lim_max ):
   ...
   units_x = 'g/cm**3'
   units_y = 'dimensionless'
   units_w = 'Msun' if weighting_type == 'mass' or weighting_type == 'dust_mass' else 'kpc**3'

   temp_dens_profile = yt.create_profile( obj,
                                          [(field_type, 'density'), (field_type, 'plasma_beta_dual')],
                                          (field_type, weighting_type),
                                          logs={(field_type, 'density'): True, (field_type, 'plasma_beta_dual'): True, (field_type, weighting_type): True},
                                          units={(field_type, 'density'): units_x, (field_type, 'plasma_beta_dual'): units_y, (field_type, weighting_type): units_w},
                                          extrema={(field_type, 'density'): (x_lim_min, x_lim_max), (field_type, 'plasma_beta_dual'): (y_lim_min, y_lim_max)},
                                          weight_field=None, n_bins=[nbin,nbin], fractional=False )

   temp_dens_profile[(field_type, weighting_type)].tofile('./bins/%s%s_Beta-Dens_%s_weighted_PhaseDiagram_z.bin'%(ds, prefix, weighting_type))
   temp_dens_profile.x.tofile('./bins/%s%s_Beta-Dens_%s_weighted_PhaseDiagram_x.bin'%(ds, prefix, weighting_type))
   temp_dens_profile.y.tofile('./bins/%s%s_Beta-Dens_%s_weighted_PhaseDiagram_y.bin'%(ds, prefix, weighting_type))

   create_PDF( ds, obj, prefix, field_type, 'density',     weighting_type, x_lim_min, x_lim_max, nbin, units_x )
   create_PDF( ds, obj, prefix, field_type, 'plasma_beta', weighting_type, y_lim_min, y_lim_max, nbin, units_y )
...
#  plot
   ...
   for idx in indices[::-1]:
      ...
      if idx == indices[-1]:
         xf  = np.fromfile('./bins/%s%s_Beta-Dens_%s_weighted_PhaseDiagram_x.bin'%(filename, prefix, weighting_type))
         yf  = np.fromfile('./bins/%s%s_Beta-Dens_%s_weighted_PhaseDiagram_y.bin'%(filename, prefix, weighting_type))
         zf  = np.fromfile('./bins/%s%s_Beta-Dens_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, weighting_type)).reshape(len(xf),len(yf))
      else:
         try:
            zf += np.fromfile('./bins/%s%s_Beta-Dens_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, weighting_type)).reshape(len(xf),len(yf))
         except Exception as e:
   ...
   grid[0].axes.set_ylabel(r'$\beta$', fontsize=FONT_SIZE)
   ...
   plot_PDF( fig, 'y', indices, code, prefix, 'plasma_beta', weighting_type, y_lim_min, y_lim_max, nbin, units_y )
   fig.savefig( './imgs_pd/fig_%s%s_Beta-Dens_%s_weighted_PhaseDiagram.png'%(fileout, prefix, weighting_type) )
   plt.close()
