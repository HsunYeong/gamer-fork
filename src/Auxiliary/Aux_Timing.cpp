...
static double Tur_Acc     [3] = { 0.0, 0.0, 0.0 };
...
void Aux_CreateTimer()
{
   ...
   for (int lv=0; lv<NLEVEL; lv++)
   {
      ...
      Timer_Tur_Advance[lv] = new Timer_t;
...
//       output header
         ...
         fprintf( File, "# Tur_Adv    : Turbulence\n" );
...
void Timing__EvolveLevel( const char FileName[], const double Time_LB_Main[][3] )
{
   ...
   double ParMPI[NLEVEL][6], Src_Advance[NLEVEL], Che_Advance[NLEVEL], SF[NLEVEL], FB_Advance[NLEVEL], Tur_Advance[NLEVEL];
...
// 1. timing for each sub-step
   if ( MPI_Rank == 0 )
   {
      ...
      fprintf( File, "---------------------------------------------------------------------------------------" );
      fprintf( File, "---------------------------------------\n" );
      fprintf( File, "%3s%4s%8s %8s%8s%8s%8s%8s%8s%8s%8s%8s%8s%8s%8s%8s%9s%9s%8s%9s%8s%8s%8s%8s%8s%9s%9s%11s%9s%8s%9s%10s%9s%11s%8s\n",
               "", "Lv", "Total", "dt", "Flu_Adv", "Gra_Adv", "Src_Adv", "Che_Adv", "SF", "FB_Adv", "Tur_Adv", "FixUp", "Flag", "Refine",
   ...
   for (int lv=0; lv<NLEVEL; lv++)
   {
      ...
      Tur_Advance[lv]    = Timer_Tur_Advance[lv]   ->GetValue();
...
//             --> because they are already included in the corresponding host functions
               ...
               fprintf( File, "%3s%4d%8.3f %8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%9.3f%9.3f%8.3f%9.3f%8.3f%8.3f%8.3f%8.3f%8.3f%9.3f%9.3f%11.3f%9.3f%8.3f%9.3f%10.3f%9.3f%11.3f%8.3f\n",
                        Comment_LB[v], lv,
                        Time_LB[lv][ 0][v], Time_LB[lv][27][v], Time_LB[lv][ 1][v], Time_LB[lv][ 2][v], Time_LB[lv][29][v], Time_LB[lv][ 3][v],
                        Time_LB[lv][28][v], Time_LB[lv][30][v], Time_LB[lv][31][v], Time_LB[lv][ 4][v],
...
//       --> because they are already included in the corresponding host functions
         Sum[lv] = dt[lv] + Flu_Advance[lv] + Gra_Advance[lv] + Src_Advance[lv] + Che_Advance[lv] +
                   SF[lv] + FB_Advance[lv] + Tur_Advance[lv] + FixUp[lv] + Flag[lv] + Refine[lv] +
...
// sum over all levels
   if ( MPI_Rank == 0 )
   {
      if ( OPT__TIMING_BALANCE )
      {
         ...
         for (int v=0; v<3; v++)
         fprintf( File, "%3s%4s%8.3f %8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%9.3f%9.3f%8.3f%9.3f%8.3f%8.3f%8.3f%8.3f%8.3f%9.3f%9.3f%11.3f%9.3f%8.3f%9.3f%10.3f%9.3f%11.3f%8.3f\n",
                  Comment_LB[v], "Sum",
                  Time_LB[0][ 0][v], Time_LB[0][27][v], Time_LB[0][ 1][v], Time_LB[0][ 2][v], Time_LB[0][29][v], Time_LB[0][ 3][v],
                  Time_LB[0][28][v], Time_LB[0][30][v], Time_LB[0][31][v], Time_LB[0][ 4][v],
      ...
      else
      {
         ...
         fprintf( File, "%3s%4s%8.3f %8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f%9.3f%9.3f%8.3f%9.3f%8.3f%8.3f%8.3f%8.3f%8.3f%9.3f%9.3f%11.3f%9.3f%8.3f%9.3f%10.3f%9.3f%11.3f%8.3f\n",
                  ...
                  SF[0], FB_Advance[0], Tur_Advance[0], FixUp[0], Flag[0], Refine[0],
...
//    _P : percentage; _IM : imbalance
      ...
      double dt_P, Flu_P, Gra_P, Src_P, Che_P, SF_P, FB_P, Tur_P, FixUp_P, Flag_P, Refine_P, Sum_P, MPI_Grid_P, Aux_P, Corr_P, Output_P, LB_P, Par_P, MPI_Par_P, libyt_P;
      double dt_IB, Flu_IB, Gra_IB, Src_IB, Che_IB, SF_IB, FB_IB, Tur_IB, FixUp_IB, Flag_IB, Refine_IB, Sum_IB, MPI_Grid_IB, Aux_IB, Corr_IB, Output_IB, LB_IB, Par_IB, MPI_Par_IB, libyt_IB;

      fprintf( File, "Summary\n" );
      fprintf( File, "---------------------------------------------------------------------------------------" );
      fprintf( File, "---------------------------------------\n" );
      fprintf( File, "%3s%5s %11s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%12s\n",
               "", "", "dt", "Flu_Adv", "Gra_Adv", "Src_Adv", "Che_Adv", "SF", "FB_Adv", "Tur_Adv", "FixUp", "Flag", "Refine",
...
//       2.1 time
         fprintf( File, "%3s%5s %11.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%12.4f\n",
                  Comment_LB[v], "Time", Time_LB[0][27][v], Time_LB[0][1][v], Time_LB[0][2][v], Time_LB[0][29][v],
                  Time_LB[0][3][v], Time_LB[0][28][v], Time_LB[0][30][v], Time_LB[0][31][v], Time_LB[0][4][v], Time_LB[0][5][v], Time_LB[0][6][v],
...
//    2.2 "max" imbalance = (Max-Ave)/Ave
      ...
      Tur_IB      = 100.0*( Time_LB[0][31][0] - Time_LB[0][31][2] ) / ((Time_LB[0][31][2]==0.0)?1.0:Time_LB[0][31][2]);
      ...
      fprintf( File, "%9s%10.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%11.3f%%\n",
               "Imbalance", dt_IB, Flu_IB, Gra_IB, Src_IB, Che_IB, SF_IB, FB_IB, Tur_IB, FixUp_IB, Flag_IB, Refine_IB,
...
//    2.3 "max" percentage
      ...
      Tur_P      = 100.0*Time_LB[0][31][0]/Everything[0];
      ...
      fprintf( File, "%3s%5s %10.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%11.3f%%\n",
               "Max", "Frac", dt_P, Flu_P, Gra_P, Src_P, Che_P, SF_P, FB_P, Tur_P, FixUp_P, Flag_P, Refine_P,
...
//    2.4 record the accumulated timing results
   ...
   else
   {
      ...
      double dt_P, Flu_P, Gra_P, Src_P, Che_P, SF_P, FB_P, Tur_P, FixUp_P, Flag_P, Refine_P, Sum_P, MPI_Grid_P;
...
//    percentage
      ...
      Tur_P      = 100.0*Tur_Advance[0]/Everything;
...
//    2.1 time
      fprintf( File, "%3s%5s %11.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%9.4f%12.4f\n",
               "", "Time", dt[0], Flu_Advance[0], Gra_Advance[0], Src_Advance[0], Che_Advance[0], SF[0], FB_Advance[0], Tur_Advance[0],
               FixUp[0], Flag[0], Refine[0], MPI_Grid, Output, Aux, LB, Corr, Par, MPI_Par, libyt, Sum[0] );

//    2.2 percentage
      fprintf( File, "%3s%5s %10.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%11.3f%%\n",
               "", "Frac", dt_P, Flu_P, Gra_P, Src_P, Che_P, SF_P, FB_P, Tur_P, FixUp_P, Flag_P, Refine_P,
               MPI_Grid_P, Output_P, Aux_P, LB_P, Corr_P, Par_P, MPI_Par_P, libyt_P, Sum_P );
      fprintf( File, "\n" );
...
void Aux_AccumulatedTiming( const double TotalT, double InitT, double OtherT )
{
   ...
   double dt_IB, Flu_IB, Gra_IB, Src_IB, Che_IB, SF_IB, FB_IB, Tur_IB, FixUp_IB, Flag_IB, Refine_IB, Sum_IB, MPI_Grid_IB;
...
// record
   ...
   fprintf( File, "----------------------------------------------------------------------------------------" );
   fprintf( File, "--------------------------------------\n" );
   fprintf( File, "%3s%5s %9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s%9s\n",
            "", "", "dt", "Flu_Adv", "Gra_Adv", "Src_Adv", "Che_Adv", "SF", "FB_Adv", "Tur_Adv", "FixUp", "Flag", "Refine",
   ...
   {
      for (int v=0; v<3; v++)
      fprintf( File, "%3s%5s %9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f\n",
               Comment_LB[v], "Time", dt_Acc[v], Flu_Acc[v], Gra_Acc[v], Src_Acc[v], Che_Acc[v], SF_Acc[v], FB_Acc[v], Tur_Acc[v],
...
//    "max" imbalance = (Max-Ave)/Ave
      ...
      Tur_IB      = 100.0*( Tur_Acc     [0] - Tur_Acc     [2] ) / ( (Tur_Acc     [2]==0.0) ? 1.0 : Tur_Acc     [2] );
      ...
      fprintf( File, "\n" );
      fprintf( File, "%9s%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%\n",
               "Imbalance", dt_IB, Flu_IB, Gra_IB, Src_IB, Che_IB, SF_IB, FB_IB, Tur_IB, FixUp_IB, Flag_IB, Refine_IB,
               MPI_Grid_IB, Output_IB, Aux_IB, LB_IB, Corr_IB, Par_IB, MPI_Par_IB, libyt_IB, Init_IB, Other_IB, Sum_IB );
      fprintf( File, "%3s%5s %8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%\n",
               Comment_LB[0], "Frac", dt_P, Flu_P, Gra_P, Src_P, Che_P, SF_P, FB_P, Tur_P, FixUp_P, Flag_P, Refine_P,
   ...
   {
      fprintf( File, "%3s%5s %9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f%9.2f\n",
               "", "Time", dt_Acc[0], Flu_Acc[0], Gra_Acc[0], Src_Acc[0], Che_Acc[0], SF_Acc[0], FB_Acc[0], Tur_Acc[0],
      ...
      fprintf( File, "%3s%5s %8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%%8.3f%%\n",
               "", "Frac", dt_P, Flu_P, Gra_P, Src_P, Che_P, SF_P, FB_P, Tur_P, FixUp_P, Flag_P, Refine_P, MPI_Grid_P,
